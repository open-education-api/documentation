...
Although the name Open Education API might suggest that all data provided by an API is open to the public, this is not the case. The implementation of the different OEAPI requests and responses requires different types of security.

A typical implementation of OEAPI exposes internal (data) service endpoints for external consumption. This can be either through additional aggregation, e.g. OEAPI gateway, or directly form the OEAPI endpoints from institutions (e.g. Leiden University, or inHolland). The next figure provides an overview of components that are usually used.

![OEAPI components to provide access to API endpoints](../_media/OEAPI_v50_components_IAA.png)
...
In the OEAPI IAA efforts we conform to the [IAA vision of SURF](https://www.surf.nl/visie-iaa-belangrijkste-trends-identiteitsstelsels)
